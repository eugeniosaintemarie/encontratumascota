...
import {
  PublicacionBadges,
  PublicacionDetalle,
} from "@/components/publicacion-badges";
...
export const PublicacionCard = memo(function PublicacionCard({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("demo-session-updated", handleSessionUpdate);
  }, []);
    ...
    >
      ...
      >
        <Image
          ...
          alt={`${publicacion.mascota.especie} ${publicacion.mascota.raza} ${publicacion.tipoPublicacion === "buscada" ? "buscado" : publicacion.tipoPublicacion === "adopcion" ? "en adopción" : "encontrado"}${publicacion.tipoPublicacion !== "adopcion" ? ` en ${publicacion.ubicacion}` : ""}`}
          fill
          loading="lazy"
      ...
      <CardContent className="flex flex-1 flex-col px-3 py-3">
        ...
        <div className="mt-auto">
          {(() => {
            const isCerrada =
              publicacion.activa === false && !publicacion.enTransito;
            const canSeeContact =
              authChecked || (publicacion.mostrarContactoPublico && !isCerrada);
                ...
                <div className="space-y-0 leading-tight rounded-lg bg-transparent p-0 overflow-hidden min-h-[84px] flex flex-col flex-1">
                  <div className="mb-1">
                    ...
                    {publicacion.esRefugio ? (
                      <button
                        onClick={() =>
                          router.push(`/refugio/${publicacion.usuarioId}`)
                        }
