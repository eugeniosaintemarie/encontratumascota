...
export async function PATCH(request: Request) {
  try {
    ...
    const { getRefugioProfileByAuthUserId, setRefugioProfile } =
    ...
    const contactoNombre = sanitizeText(
      body?.contactoNombre ?? existingProfile?.contactoNombre ?? "",
    );
    const contactoTelefono = sanitizePhone(
      body?.contactoTelefono ?? existingProfile?.contactoTelefono ?? "",
    );
    const contactoEmail = sanitizeEmail(
      body?.contactoEmail ?? existingProfile?.contactoEmail ?? "",
    );
    const mostrarContactoPublico = Boolean(
      body?.mostrarContactoPublico ??
      existingProfile?.mostrarContactoPublico ??
      false,
    );
    const ubicacion = sanitizeText(
      body?.ubicacion ?? existingProfile?.ubicacion ?? "",
    );
