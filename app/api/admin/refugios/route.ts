...
export async function PATCH(request: Request) {
  try {
    ...
    const nombreRefugio = body?.nombreRefugio ? String(body.nombreRefugio).trim() : null

    if (!authUserId) {
      return NextResponse.json({ error: "authUserId es obligatorio" }, { status: 400 })
    }

    const { setRefugioProfile } = await import("@/lib/actions/refugios")
    const profile = await setRefugioProfile({ authUserId, esRefugio, nombreRefugio })
