...
export function getPublicacionInfo(publicacion: Publicacion): PublicacionInfo {
  ...
  if (esMestizo) {
    ...
    const padre = mascota.padreRaza
      ? razasLabels[mascota.padreRaza] || mascota.padreRaza
      : "?";
    const madre = mascota.madreRaza
      ? razasLabels[mascota.madreRaza] || mascota.madreRaza
      : "?";
    madreRaza = `♀ ${madre}`;
    padreRaza = `♂ ${padre}`;
  ...
  if (publicacion.tipoPublicacion === "adopcion") {
    ...
    publicacion.fechaEncuentro &&
    publicacion.fechaEncuentro.getFullYear() > 1970
  ) {
