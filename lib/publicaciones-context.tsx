...
interface PublicacionesContextType {
  ...
  agregarPublicacion: (
    publicacion: Omit<Publicacion, "id" | "fechaPublicacion">,
  ) => void;
...
const PublicacionesContext = createContext<PublicacionesContextType | null>(
  null,
);
...
export function PublicacionesProvider({ children }: { children: ReactNode }) {
  ...
  const fetchPublicaciones = useCallback(
    async (options?: { skipShuffle?: boolean; includeInactive?: boolean }) => {
      try {
        ...
        try {
          if (
            ...
            (typeof window !== "undefined" &&
              window.location.pathname === "/reunidas")
          ) {
  ...
  const cerrarPublicacion = useCallback(
    async (
      id: string,
      motivo: "encontrado_dueno" | "adoptado" | "en_transito" | "otro",
      transitoContacto?: { nombre: string; telefono: string; email: string },
  ...
  const agregarPublicacion = useCallback(
    async (publicacion: Omit<Publicacion, "id" | "fechaPublicacion">) => {
      try {
  ...
  const actualizarPublicacion = useCallback(
    async (id: string, datos: Partial<Publicacion>) => {
      try {
...
export function usePublicaciones() {
  ...
  if (!context) {
    throw new Error(
      "usePublicaciones debe usarse dentro de PublicacionesProvider",
    );
  }
  return context;
}
