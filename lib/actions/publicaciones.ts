...
async function enrichPublicacionesWithRefugioProfile(
  publicaciones: Publicacion[],
): Promise<Publicacion[]> {
  if (publicaciones.length === 0) return publicaciones;

  const { getRefugioProfileMapByAuthUserIds } =
    await import("@/lib/actions/refugios");
  const profileMap = await getRefugioProfileMapByAuthUserIds(
    publicaciones.map((p) => p.usuarioId),
  );
  ...
  return publicaciones.map((pub) => {
    ...
    return {
      ...
      contactoNombre:
        profile.esRefugio && profile.nombreRefugio
          ? profile.nombreRefugio
          : pub.contactoNombre,
    };
  });
}

async function enrichPublicacionWithRefugioProfile(
...
export async function getPublicaciones(
  ...
  try {
    ...
    if (filtros?.tipoPublicacion) {
      conditions.push(
        eq(publicaciones.tipoPublicacion, filtros.tipoPublicacion),
      );
    }
  ...
  if (isDemo) {
    const mockFiltradas = mockPublicaciones.filter((pub) => {
      ...
      if (
        ...
        pub.tipoPublicacion !== filtros.tipoPublicacion
      )
        return false;
      if (
        filtros?.especie &&
        filtros.especie !== "todos" &&
        pub.mascota.especie !== filtros.especie
      )
        return false;
      if (
        filtros?.raza &&
        filtros.raza !== "todos" &&
        pub.mascota.raza !== filtros.raza
      )
        return false;
      if (
        filtros?.sexo &&
        filtros.sexo !== "todos" &&
        pub.mascota.sexo !== filtros.sexo
      )
        return false;
      if (
        filtros?.ubicacion &&
        !pub.ubicacion.toLowerCase().includes(filtros.ubicacion.toLowerCase())
      )
        return false;
...
export async function getPublicacionById(
  ...
  if (isDemo) {
    ...
    if (mockPub)
      return mockPub.esRefugio
        ? mockPub
        : { ...mockPub, usuarioId: "demo-admin" };
  }
...
function mapRowToPublicacion(
  row: typeof publicacionesTable.$inferSelect,
): Publicacion {
  return {
    id: row.id,
